import { A as e, P as t, dt as n, j as r, xt as i } from "./find-cursor-CaGrVs7z.js";
...
function A(e, t, r, i, a, o, s = null, c = null, l = null, u = null) {
	let d = r + a / 2, f = i + o / 2;
	{
		let d = t.toLowerCase(), f = E[d] ?? d;
		if ((D.has(f) || O.has(f) || k.has(f)) && n(e, f, r, i, a, o, [
			s,
			c,
			l,
			u
	...
	switch (t.toLowerCase()) {
		...
		case "curvedrightarrow": {
			let t = Math.min(a, o), n = o / 2, u = 5e4 * o / t, d = Math.min(u, Math.max(0, c ?? 5e4)), f = t * Math.min(d, Math.max(0, s ?? 25e3)) / 1e5, p = t * d / 1e5, m = n - (f + p) / 4, h = (2 * m) ** 2 - f ** 2, g = 1e5 * (Math.sqrt(Math.max(0, h)) * a / (2 * m)) / t, _ = t * Math.min(g, Math.max(0, l ?? 25e3)) / 1e5, v = Math.sqrt(Math.max(0, a * a - _ * _)) * m / a, y = m + f, b = m + v, x = y + v, S = (p - f) / 2, C = b - S, w = x + S, E = o - p / 2, D = a - _, O = Math.atan2(_, v), k = -O, A = Math.PI - O;
			e.moveTo(r, i + m), T(e, r, i + m, a, m, Math.PI, k), e.lineTo(r + D, i + C), e.lineTo(r + a, i + E), e.lineTo(r + D, i + w), e.lineTo(r + D, i + x), T(e, r + D, i + x, a, m, A, O), e.closePath();
			break;
		}
		case "curvedleftarrow": {
			let t = Math.min(a, o), n = o / 2, u = 5e4 * o / t, d = Math.min(u, Math.max(0, c ?? 5e4)), f = t * Math.min(d, Math.max(0, s ?? 25e3)) / 1e5, p = t * d / 1e5, m = n - (f + p) / 4, h = (2 * m) ** 2 - f ** 2, g = Math.sqrt(Math.max(0, h)) * a / (2 * m), _ = 1e5 * g / t, v = t * Math.min(_, Math.max(0, l ?? 25e3)) / 1e5, y = Math.sqrt(Math.max(0, a * a - v * v)) * m / a, b = m + f, x = m + y, S = b + y, C = (p - f) / 2, w = x - C, E = S + C, D = o - p / 2, O = v, k = Math.atan2(v, y), A = f / 2, j = Math.atan2(A, g), M = j - k, N = k - j, P = -j;
			e.moveTo(r, i + D), e.lineTo(r + O, i + w), e.lineTo(r + O, i + x);
			let F = T(e, r + O, i + x, a, m, k, M);
			T(e, F.x, F.y, a, m, P, N), e.lineTo(r + O, i + E), e.closePath();
			break;
		}
		case "curveduparrow": {
			let t = Math.min(a, o), n = a / 2, u = 5e4 * a / t, d = Math.min(u, Math.max(0, c ?? 5e4)), f = t * Math.min(1e5, Math.max(0, s ?? 25e3)) / 1e5, p = t * d / 1e5, m = n - (f + p) / 4, h = (2 * m) ** 2 - f ** 2, g = Math.sqrt(Math.max(0, h)) * o / (2 * m), _ = 1e5 * g / t, v = t * Math.min(_, Math.max(0, l ?? 25e3)) / 1e5, y = Math.sqrt(Math.max(0, o * o - v * v)) * m / o, b = m + f, x = m + y, S = b + y, C = (p - f) / 2, w = x - C, E = S + C, D = a - p / 2, O = v, k = Math.atan2(v, y), A = f / 2, j = Math.atan2(A, g), M = j - k, N = k - j, P = Math.PI / 2 - k, F = Math.PI / 2 - j;
			e.moveTo(r + D, i), e.lineTo(r + E, i + O), e.lineTo(r + S, i + O);
			let I = T(e, r + S, i + O, m, o, P, N);
			T(e, I.x, I.y, m, o, F, M), e.lineTo(r + w, i + O), e.closePath();
			break;
		}
		case "curveddownarrow": {
			let t = Math.min(a, o), n = a / 2, u = 5e4 * a / t, d = Math.min(u, Math.max(0, c ?? 5e4)), f = t * Math.min(1e5, Math.max(0, s ?? 25e3)) / 1e5, p = t * d / 1e5, m = n - (f + p) / 4, h = (2 * m) ** 2 - f ** 2, g = Math.sqrt(Math.max(0, h)) * o / (2 * m), _ = 1e5 * g / t, v = t * Math.min(_, Math.max(0, l ?? 25e3)) / 1e5, y = Math.sqrt(Math.max(0, o * o - v * v)) * m / o, b = m + f, x = m + y, S = b + y, C = (p - f) / 2, w = x - C, E = S + C, D = a - p / 2, O = o - v, k = Math.atan2(v, y), A = f / 2, j = Math.atan2(A, g), M = 3 * Math.PI / 2 + k;
			3 * Math.PI / 2 - j, j - Math.PI / 2, Math.PI / 2 - j, e.moveTo(r + D, i + o), e.lineTo(r + w, i + O), e.lineTo(r + x, i + O), T(e, r + x, i + O, m, o, M, -k), e.lineTo(r + b, i), T(e, r + b, i, m, o, 3 * Math.PI / 2, k), e.lineTo(r + E, i + O), e.closePath();
			break;
		}
...
function F(e, t, n, r, a, o, s) {
	...
	let { lw: c, halfW: l, len: u } = j(a, o, s), d = i(o.color);
...
function L(n, i, a = {}) {
	...
	let o = a.firstContentSi ?? 0, s = a.lastDrawnSi ?? n.length - 1, c = a.minPerGap ?? -Infinity, l = a.isGapChar ?? t, u = a.isWhitespace ?? I, d = a.seaClusterGaps ?? !1, f = [];
...
//#endregion
export { F as a, A as c, d, u as f, L as i, S as l, B as n, N as o, f as p, z as r, P as s, V as t, p as u };
