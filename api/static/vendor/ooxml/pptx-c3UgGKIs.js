...
async function Me(e, t, n) {
	...
	let a = n.width / (n.slideWidthEmu / _);
	await n.drawBase();
	...
	let f = null, p = null, m = () => {
		...
		let { x: n, y: r } = g(t.clientX, t.clientY), i = v(n, r);
		i && (i.kind === "seek" ? (y = {
			state: i.state,
			wasPlaying: !i.state.media.paused
		}, i.state.media.pause(), b(i.state, i.fraction), e.setPointerCapture(t.pointerId), t.preventDefault()) : i.state.media.paused ? x(i.state) : i.state.media.pause());
	}, C = (e) => {
		...
		if (y) {
			let e = K(y.state), n = Math.max(0, Math.min(1, (t - e.x) / e.w));
			b(y.state, n);
		}
	}, w = () => {
		p = null;
		...
		if (!y) return;
		let { wasPlaying: n, state: r } = y;
		y = null, e.releasePointerCapture(t.pointerId), n && x(r);
	};
	return c.length > 0 && (e.addEventListener("pointerdown", S), e.addEventListener("pointermove", C), e.addEventListener("pointerleave", w), e.addEventListener("pointerup", T), e.addEventListener("pointercancel", T), e.style.cursor = "pointer", h()), {
		play(e) {
			for (let t of c) (!e || t.el.mediaPath === e) && x(t);
		},
		pause(e) {
...
var Y = class e {
	...
	constructor(e, t, n) {
		this._worker = e, this._mode = t, this._bridge = new te(this._worker, {
			...
			toError: (e) => !("protocol" in e) && e.kind === "error" ? C(e) : void 0
		});
	...
	async renderSlide(e, t, r = {}) {
		...
		try {
			...
			let o = r.dpr ?? S(), s = r.width ?? ((b(e) ? e.offsetWidth : 0) || 960);
	...
	destroy() {
		this._slidePullClient?.cancelAll(), this._bridge.terminate(), this._slides?.clear(), this._slides = null, this._slidePullClient = null, this._preflight = null, this._resourceFailure = null, this._slidePartIndex = null, this._rawParts.clear(), this._googleFontFaces.length > 0 && (ye(this._googleFontFaces), this._googleFontFaces = []), w(this._fetchImage), d(this._fetchImage);
	}
};
//#endregion
...
var Q = Symbol("PptxViewer.borrowedPresentation"), mt = {
	...
	async _fit(e) {
		...
		if (!t) return;
		let n = E({
			contentWidth: this.engine.slideWidth / _,
			contentHeight: this.engine.slideHeight / _,
	...
	_slideWidthPx() {
		return this._pres.slideWidth / _ * this._scale;
	}
	_slideHeightPx() {
		return this._pres.slideHeight / _ * this._scale;
	}
	_fitWidthPx() {
	...
	_baseScale() {
		...
		let e = this._fitWidthPx(), t = this._pres.slideWidth / _;
	...
	_fit(e) {
		if (!this._pres || this._pres.slideCount === 0) return;
		let t = E({
			contentWidth: this._pres.slideWidth / _,
			contentHeight: this._pres.slideHeight / _,
