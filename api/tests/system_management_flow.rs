...
#[tokio::test]
async fn system_dashboard_keeps_ssr_rollback_when_shared_shell_is_disabled() {
    ...
    assert!(body.contains("system-grid"));
...
#[tokio::test]
async fn system_releases_keep_ssr_rollback_when_shared_shell_is_disabled() {
    let pool = test_pool().await;
    let initialized = bootstrap_admin_session(&pool).await;
    let app = build_router(AppState::new(test_settings(), Some(pool)));
...
#[tokio::test]
async fn system_permissions_keep_ssr_rollback_when_shared_shell_is_disabled() {
    let pool = test_pool().await;
    let initialized = bootstrap_admin_session(&pool).await;
    let app = build_router(AppState::new(test_settings(), Some(pool)));
    ...
    assert_eq!(response.status(), StatusCode::OK);
    let body = response_body(response).await;
    assert!(body.contains("全部权限点"));
    assert!(body.contains("data-permission-tree"));
    assert!(!body.contains("/web/app/assets/"));
}
