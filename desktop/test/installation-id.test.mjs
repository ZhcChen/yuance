...
test("installation ID is stable and owner-only", async (t) => {
  ...
  const first = await loadOrCreateInstallationId({ fs, filePath, platform: process.platform, randomUUID: () => FIRST });
  const second = await loadOrCreateInstallationId({ fs, filePath, platform: process.platform, randomUUID: () => SECOND });
...
test("installation ID atomically replaces a truncated file", async (t) => {
  ...
  assert.equal(
    await loadOrCreateInstallationId({ fs, filePath, platform: process.platform, randomUUID: () => SECOND }),
    SECOND,
  );
