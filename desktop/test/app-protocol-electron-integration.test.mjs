...
function validReport() {
  const renderer = {
    ...
    title: "元策",
    bodyText: "需要登录",
    bridgeSchemaVersion: 14,
    bridgeState: "unauthenticated",
    desktopStage: "authorization",
    theme: "light",
    rootBackgroundColor: "rgb(237, 241, 245)",
    resourceUrls: [
  ...
  return {
    ...
    stageSequence: ["authorization", "authorization"],
    rendererReadinessCount: 2,
    mainDocumentNavigationCount: 2,
...
test("rejects protocol, renderer, IPC, permission, profile, and network regressions", () => {
  const mutations = [
    ...
    (value) => { value.reloadedRenderer.bridgeSchemaVersion = 11; },
    (value) => { value.stageSequence = ["bootstrap", "authorization"]; },
    (value) => { value.rendererReadinessCount = 1; },
    (value) => { value.mainDocumentNavigationCount = 3; },
    (value) => { value.initialRenderer.theme = "dark"; },
    (value) => { value.reloadedRenderer.rootBackgroundColor = "rgb(255, 255, 255)"; },
