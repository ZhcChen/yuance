...
export function registerAuthCommandHandlers({ ipcMain, assertSender, getRuntime, getNetworkCoordinator, openExternal } = {}) {
  ...
  const handlers = {
    [AUTH_CHANNELS.authorize]: command((runtime) => runtime.authorize({ openExternal })),
    [AUTH_CHANNELS.retry]: command((runtime) => {
      ...
      return runtime.authorize({ openExternal });
    }),
