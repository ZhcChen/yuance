    ...
    <section className="work-item-page">
      ...
      {canEditPrimaryPost && !isDeleted ? <Modal wide open={activePanel === 'edit'} title="编辑工作项" onClose={closePanel} footer={<><Button variant="secondary" disabled={mutationBusy} onClick={closePanel}>取消</Button><Button type="submit" form="work-item-edit-form" loading={editSubmitting}>保存修改</Button></>}>
        <form id="work-item-edit-form" className="work-item-action-form work-item-edit-form" onSubmit={async (event) => { if (await onSubmitEdit(event)) setActivePanel(null); }}>
          <div className="work-item-edit-fields">
            <div className="work-item-form-field"><label htmlFor="work-item-edit-title">标题</label><TextInput id="work-item-edit-title" name="title" value={editForm.title} onChange={onChangeEdit} required /></div>
            ...
            <div className="work-item-form-field"><label htmlFor="work-item-edit-assignee">处理人</label><Select id="work-item-edit-assignee" name="assigneeUsername" searchable searchPlaceholder="搜索处理人" value={editForm.assigneeUsername} onChange={onChangeEdit}><option value="">未分配</option>{assigneeOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</Select></div>
            ...
            {item.item_type === 'task' ? (
              <div className="work-item-form-field"><label htmlFor="work-item-edit-parent">父级需求</label><Select id="work-item-edit-parent" name="parentItemKey" value={editForm.parentItemKey} onChange={onChangeEdit}><option value="">不关联</option>{parentOptions.map((option) => <option key={option.key} value={option.key}>{option.key} · {option.title}</option>)}</Select></div>
            ) : null}
          </div>
