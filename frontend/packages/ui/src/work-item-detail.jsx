    ...
    <section className="work-item-page">
      <header className="work-item-hero">
        ...
        <div className="work-item-title-row">
          <div><h1 aria-label={`${item.key} · ${item.title}`}><span>{item.title}</span><span className="work-item-title-meta"><span className="work-kind" data-kind={item.item_type}>{item.item_type === 'requirement' ? '需求' : item.item_type === 'bug' ? 'Bug' : '任务'}</span><span className="work-item-key">({item.key})</span></span></h1></div>
