...
import {
  ...
  projectCycleApiPath,
  projectMemberBatchApiPath,
  projectMemberCandidatesApiPath,
...
test('work item attachment preview uses the fixed path and strips private fields', async () => {
  ...
  const client = createApiClient({ request: async (url) => {
    ...
    return {
      ...
      content_url: '/api/v1/work-items/YCE-TASK-2/attachments/7/preview/content',
      download_url: '/api/v1/work-items/YCE-TASK-2/attachments/7/download-url',
    };
  } });
  const result = await client.getWorkItemAttachmentPreview('YCE-TASK-2', 7);
  assert.deepEqual(calls, ['/api/v1/work-items/YCE-TASK-2/attachments/7/preview']);
  assert.equal(result.preview.kind, 'document');
  assert.equal(Object.hasOwn(result.attachment, 'object_key'), false);
});
...
test('work item comment attachment deletion fixes the editor context header', async () => {
  const { client, calls, writes } = createRecordedClient();
  await client.deleteWorkItemCommentAttachment('YCE-TASK-2', 9, 7);
  assert.deepEqual(writes, ['prepare']);
  assert.deepEqual(calls[0], {
    url: '/api/v1/work-items/YCE-TASK-2/comments/9/attachments/7',
    options: { method: 'DELETE', headers: { 'x-yuance-editor-context': 'work-item-comment-edit' } },
  });
});
...
test('closeWorkItem uses the fixed write endpoint', async () => {
  const { client, calls, writes } = createRecordedClient();
  await client.closeWorkItem('YCE-TASK/2');
  assert.equal(writes.length, 1);
  assert.equal(calls[0].url, '/api/v1/work-items/YCE-TASK%2F2/close');
  assert.equal(calls[0].options.method, 'POST');
});
...
test('project detail and member methods use one shared write contract', async () => {
  ...
  await client.getProjectMemberCandidates('YCE/1');
  ...
  await client.addProjectMembers('YCE/1', { usernames: ['bob', 'carol'], memberRole: 'viewer' });
  ...
  assert.equal(projectMemberCandidatesApiPath('YCE/1'), '/api/v1/projects/YCE%2F1/members/candidates');
  assert.equal(projectMemberBatchApiPath('YCE/1'), '/api/v1/projects/YCE%2F1/members/batch');
  assert.deepEqual(writes, ['prepare', 'prepare', 'prepare', 'prepare', 'prepare']);
  assert.deepEqual(calls.map(({ url, options }) => [url, options.method || 'GET', options.body ? JSON.parse(options.body) : undefined]), [
    ['/api/v1/projects/YCE%2F1', 'GET', undefined],
    ['/api/v1/projects/YCE%2F1/members', 'GET', undefined],
    ['/api/v1/projects/YCE%2F1/members/candidates', 'GET', undefined],
    ...
    ['/api/v1/projects/YCE%2F1/members/batch', 'POST', { usernames: ['bob', 'carol'], member_role: 'viewer' }],
