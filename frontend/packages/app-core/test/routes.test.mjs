...
test('parseAppRoute supports owner-aware project detail tabs', () => {
  ...
  assert.equal(buildProjectDetailPath({ owner: 'app', projectKey: 'YCE', tab: 'files' }), '/web/app/projects/YCE?tab=time');
  assert.equal(buildProjectCycleDetailPath({ owner: 'web', projectKey: 'YCE', cycleId: 7 }), '/web/projects/YCE/cycles/7');
  assert.equal(buildProjectDetailPath({ owner: 'app', projectKey: 'YCE', tab: 'resources' }), '/web/app/projects/YCE?tab=time');
  assert.deepEqual(parseAppRoute('/web/app/projects/YCE', '?tab=files'), {
    id: 'project-detail', owner: 'app', pathname: '/web/app/projects/YCE', search: '?tab=files', projectKey: 'YCE', tab: 'time', title: '项目详情',
  });
  assert.deepEqual(parseAppRoute('/web/app/projects/YCE', '?tab=resources'), {
