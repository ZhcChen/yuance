...
try {
    ...
    New-TestRelease "0.1.5" "upgraded"
    & $Installer -Version "0.1.5" -ReleaseDir $ReleaseDir -InstallDir $InstallDir | Out-Null
    Assert-Equal (Get-Content -LiteralPath (Join-Path $InstallDir "fixture-marker.txt") -Raw) "upgraded"

    New-TestRelease "0.1.5" "checksum-failure"
    $currentTarget = (& $Installer -DetectOnly).Trim()
    Add-Content -LiteralPath (Join-Path $ReleaseDir "yuance-agent-v0.1.5-$currentTarget.zip") -Value "tampered"
    try {
        & $Installer -Version "0.1.5" -ReleaseDir $ReleaseDir -InstallDir $InstallDir | Out-Null
        throw "校验和错误时安装器意外成功"
    } catch {
    ...
    New-TestRelease "0.1.3" "missing" "missing-binary"
    try {
    ...
    New-TestRelease "0.1.3" "self-check" "self-check-fails"
    try {
        & $Installer -Version "0.1.3" -ReleaseDir $ReleaseDir -InstallDir $InstallDir | Out-Null
