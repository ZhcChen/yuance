...
- U2.1 已注册通用 MCP `code-review-graph`，CLI 自动定位图数据与 MCP stdio 冒烟均通过；全新 Codex 会话已加载 MCP 并成功调用 `query_graph_tool`，工具面运行时验证通过。
...
- U3.2 已写入试点 review 与 solution，并更新 AGENTS.md、工具文档和本计划状态；全部执行单元完成。
