...
**Means:** 以资料附件作为 SVG 的唯一渲染入口，在附件上传完成阶段进行内容校验，并让已有资料附件预览链路明确支持 SVG；本次不开放正文任意内联 SVG（KTD1）。
...
为资料库增加可安全使用的 SVG 流程图附件能力。用户上传 SVG 后，可以在资料详情中通过现有附件预览入口直接查看图形，也可以继续下载原文件。正文不再把 SVG 源码作为唯一可用展示方式，正文引用通过附件图片占位符完成。
...
- **R6**：正文可以通过现有附件图片占位符引用 SVG；不开放任意正文内联 SVG。
...
- **KTD1：SVG 以附件预览为主，不全局放开正文内联 SVG。** 资料正文的 HTML 清洗策略继续隔离 SVG；正文通过受控附件图片占位符引用。
...
- **Approach:** 确认 SVG 走图片预览策略和受鉴权 content URL；补齐资料场景而非改变所有 HTML 清洗规则；图片占位符复用现有附件 ID/预览入口；为下载、加载失败、非法附件响应补充状态覆盖。
