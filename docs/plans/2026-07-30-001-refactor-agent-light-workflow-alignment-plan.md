...
本计划用于把当前项目的 CE 轻工作流调整为兼容 `agent-light-workflow` Codex 版轻工作流的结构，并保留已有工作流文档资产。

推荐路线是“兼容增强”，不是直接覆盖：新增 `docs/prompts/`、`docs/reviews/` 和各类 `TEMPLATE.md`，并轻量修订 `AGENTS.md` 说明轻工作流入口。这样既满足 `agent-light-workflow` 的目录与提示词约定，也不破坏当前项目已经形成的 CE 执行习惯。
...
- 当前项目曾把提示词放在旧快捷入口目录中，和 Codex 版 `docs/prompts/` 结构不一致。
...
- 当前 `AGENTS.md` 明确采用 CE 作为主工作流，和原版“不引入隐藏 runtime 或专用目录”的定位不同。

因此，调整目标不是改业务代码，而是让仓库同时满足：

1. Codex 直接读取 `AGENTS.md` 后能理解并执行轻工作流。
2. `agent-light-workflow` 的预期目录和模板资产存在。
3. 旧快捷入口清理后，`docs/prompts/` 成为唯一提示词资产，降低后续漂移风险。
...
- R5. 更新 `AGENTS.md`，明确 `docs/prompts/` 是唯一提示词资产，避免同一阶段提示词存在多份副本。
...
- `agent-light-workflow` README（`549e27a`）：原版定位是纯文档工作流 + 薄初始化脚本，不提供专用 runtime 或 harness 目录。
...
- **采用兼容增强，而不是重写当前 CE 文档体系。** 当前项目已有大量 CE 文档资产，直接替换历史文档收益低；新增并保留 `docs/prompts/` 可满足 Codex 版轻工作流结构。
- **把 `docs/prompts/` 定义为唯一参考提示词资产。** 这样和 `agent-light-workflow` 的“提示词资产而非隐藏 runtime”定位兼容，并避免多目录提示词漂移。
...
- 是否保留旧快捷入口：后续清理中已决定删除，统一收敛到 `docs/prompts/`。
...
- `docs/prompts/*.md` 是否完全照搬外部模板文案，还是吸收本项目既有 CE 细节后本地化：执行时按 diff 可读性决定，但必须保留原版“可复制参考提示词”的定位。
...
- 在 `README.md` 中说明 `docs/prompts/` 是唯一提示词资产；如果使用 Codex 原生协作，可直接复制 `docs/prompts/*.md` 内容。
...
**Goal:** 让根入口文档明确本项目兼容 `agent-light-workflow`，并解释 `docs/prompts/` 的规范源地位。
...
**Approach:**
- 在“工作模式”中把当前项目定义为“基于 agent-light-workflow 的 CE 轻工作流”或等价表述。
- 在“产物约定”中补充：
  - `docs/reviews/`：重要改动复核与验证记录。
  - `docs/prompts/`：唯一轻工作流参考提示词资产。
...
**Verification:**
- `AGENTS.md` 明确 `docs/prompts/` 的规范源地位，且不再指向已清理的旧快捷入口。
...
- **State lifecycle risks:** 多份提示词资产同时存在会出现内容漂移；当前已收敛为 `docs/prompts/` 单一提示词目录。
...
| Risk | Mitigation |
|------|------------|
| 多目录提示词内容漂移 | 清理旧快捷入口，只保留 `docs/prompts/` 作为提示词资产。 |
...
- 若未来需要引入新的命令入口，应另开 brainstorm / plan；本计划不再保留平行提示词目录。

## Completion Notes

- 已补齐 `docs/prompts/`、`docs/reviews/` 和四类 `TEMPLATE.md`。
- 已修订 `AGENTS.md`，明确 `agent-light-workflow` 兼容层和 Codex 参考提示词入口。
...
- 2026-07-30 跟进修正：明确 `AGENTS.md` 与 `docs/prompts/` 是工作流规则规范源。
- 2026-07-30 跟进修正：Git 工作流从硬编码 `main` 调整为当前检出的协作分支，避免与当前 `dev` 分支实践冲突。
- 2026-07-30 跟进清理：删除旧快捷入口提示词目录，避免与 `docs/prompts/` 重复。
...
- Related local prompts: `docs/prompts/`
