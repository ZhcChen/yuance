...
| `docs/plans/2026-08-01-001-feat-d1-device-session-credential-plan.md` | D1 设备认证子计划/RFC | `active` | Browser 批准、device session、可恢复 refresh rotation、撤销和 Desktop 主进程安全凭证存储；不包含 renderer、SSE、文件或发行。 | 实现与本地验证已完成，等待三平台 Desktop Security workflow 后收口；下一切片只规划 `app://` 安全宿主。 |
...
| D1 / D2：Electron 安全宿主与功能对齐 | `active`（设备认证基础待 CI 收口） | Browser 批准、独立 device session、可恢复 refresh rotation、撤销、安全凭证存储、主进程 coordinator 和真实 Electron headless integration 已落地；当前 Desktop 仍以远端 Web 页面为主。 | 先通过 macOS/Windows/Linux Desktop Security workflow；随后只规划 `app://` 安全宿主。 |
