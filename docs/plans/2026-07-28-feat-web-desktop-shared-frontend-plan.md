...
| `docs/plans/2026-07-31-001-refactor-w4-shared-javascript-layer-plan.md` | W4 重构子计划/RFC | `active` | 共享 JavaScript 层提炼；已完成边界冻结和 `frontend/` workspace 骨架，尚未迁移业务代码。 | 继续按该子计划执行 API client、app-core、ui 和 Browser 回接；未完成前不启动 Desktop renderer、`app://`、device-session 或离线能力。 |
...
| W4：共享 JavaScript 层提炼 | `active`（Unit 1-2 已完成） | `docs/plans/2026-07-31-001-refactor-w4-shared-javascript-layer-plan.md` 已定义抽取边界、包依赖图、Browser transport 留存和迁移顺序；`frontend/packages/*` 骨架与边界检查已创建，尚未迁移业务代码。 | 继续按 W4 子计划执行纯 API client、app-core、ui 提炼和 Web 回接验证。 |
...
**当前状态：** `active`（Unit 1-2 已完成）

**启动门槛：** 不以“应用壳 + 只读列表/详情”启动共享包抽取。工作项协作闭环已经通过 Browser E2E 覆盖编辑、handoff、评论和附件上传/下载，可作为 W4 的首个评估输入。`docs/plans/2026-07-31-001-refactor-w4-shared-javascript-layer-plan.md` 已作为 W4 子计划/RFC，明确抽取边界、包依赖图、保留在 Browser 宿主内的 Cookie/CSRF transport、以及不纳入本轮的富文本/预览/离线能力。2026-07-31 已完成 Unit 1 边界冻结和 Unit 2 `frontend/` workspace 骨架；后续仍需按子计划迁移纯 API client、app-core、ui 并完成 Web 回接验证。
