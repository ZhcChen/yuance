...
- `desktop/src/main.mjs` 的正式构建只加载 `app://yuance/` 内置 renderer，并通过受限 preload 暴露脱敏宿主状态与原生通知；开发态仍可显式加载独立 loopback Vite renderer。
...
| `docs/plans/2026-08-01-002-feat-d1-app-protocol-secure-host-plan.md` | D1 `app://` 安全宿主子计划/RFC | `completed` | 内置 renderer、受限协议、CSP/导航、最小 preload sender 校验、Desktop composition root 与本地 Shell；不包含业务网络、文件 transfer 或发行。 | 已由本地完整矩阵、实际 macOS bundle 和三平台 Desktop Security Gate 收口。 |
| `docs/plans/2026-08-02-001-feat-d1-desktop-network-sse-plan.md` | D1 Desktop Network/SSE 子计划/RFC | `completed` | 固定 endpoint enrollment、正式 credential runtime、主进程 Device Bearer REST/fetch-stream SSE、撤销断流与脱敏 renderer 状态；不包含文件或完整业务迁移。 | 已由本地完整矩阵、正式包网络 smoke 和三平台 Desktop Security Gate 收口。 |
| `docs/plans/2026-08-02-002-feat-d1-desktop-file-capability-transfer-plan.md` | D1-C 文件 Capability/Transfer 子计划/RFC | `completed` | 主进程短期文件 capability、受控 transfer grant、上传/下载执行器、IPC 最小合约和三平台文件系统负向验证；不开放业务 API 或完整业务 UI。 | 已由本地完整矩阵、正式包文件 smoke 和三平台四 runner Desktop Security Gate 收口；D2 是唯一下一 Desktop 子计划。 |
...
| D1 / D2：Electron 安全宿主与功能对齐 | `active`（D1 completed，D2 planned） | 设备认证、`app://` 安全宿主、Desktop Network/SSE 和文件 Capability/Transfer 均已落地；正式包通过协议、网络、文件、生命周期、凭证和三平台四 runner Gate。 | D2 Desktop 业务功能对齐是唯一下一 Desktop 子计划；发行 Gate 继续保持独立。 |
| G-DIST / D3 / D4：更新与离线能力 | `pending` | 未启动。 | 作为 D2 后独立 Gate 或离线专项，不阻塞 W3。 |

### Desktop 六阶段路线图

Desktop 后续工作统一按下表管理。D1-A 至 D1-C 已完成；D2 是唯一下一 Desktop 阶段，其余阶段只冻结目标、依赖和退出条件。进入某阶段前必须基于前一阶段的真实实现另建可执行子计划，不把六个阶段作为单个执行目标，也不并行突破安全边界。

| 顺序 | 阶段 | 当前状态 | 前置依赖 | 本阶段退出条件 | 后续子计划入口 |
|---|---|---|---|---|---|
| 1 | D1-A：`app://` 安全宿主 | `completed` | D1 设备认证、W4 共享层已完成 | 三平台正式构建加载同一内置 Shell；协议、CSP、导航、权限、IPC sender 与打包资源负向测试通过；正式态不再加载远端 `/web` | `docs/plans/2026-08-01-002-feat-d1-app-protocol-secure-host-plan.md`；review：`docs/reviews/2026-08-02-d1-app-protocol-secure-host-review.md` |
...
| 4 | D2：Desktop 业务功能对齐 | `planned` | D1-A 至 D1-C 全部通过 review | 共享应用壳和选定 W3/W4 feature 在 Desktop 完成读写、通知、文件与异常恢复对齐；无第二套业务实现 | 下一步新建 `docs/plans/YYYY-MM-DD-*-desktop-feature-parity-plan.md` |
...
| 本地 renderer 安全 | 固定 `app://` origin、资源 manifest、深链接、CSP `connect-src 'none'`、导航/重定向、顶层 frame、权限和 IPC sender 规则；Desktop 网络只在主进程建立。 | 防止远端页面、renderer 脚本或非受信 frame 绕过主进程 operation registry 调用原生/网络能力。 |
...
| 服务端登记与网络 | 第一期只支持构建内固定官方 HTTPS origin，并从固定 discovery/path registry 确认 server instance 和协议能力；生产包不得由环境变量、renderer、网页、CLI 或本地可写文件静默改写 endpoint。私有化/多服务器、签名 enrollment、MDM 根、证书 pinning和私有 CA 另立产品与安全计划。 | 禁止由任意远端页面或本地配置决定 IPC/凭证信任；未来切换 server 必须先清理旧凭证、流和缓存，再重新设备授权。 |
...
| Desktop SSE 与网络 | D1-B 固定 Device Session fetch-stream Bearer 控制流、重连/轮换/代理行为、系统代理/PAC、TLS 默认验证和禁止证书错误绕过；连接绑定 device/user/authorization version，且设备撤销、用户禁用、version 变化和 token 到期会在可断言的 `revocation_close_deadline` 内关闭。控制流不含业务数据；权限/角色/成员资格变化后的业务流断开由 D2 逐 feature 验收。 | 防止 `app://` 误用浏览器 Cookie/EventSource 假设、以降级方式绕过 TLS，或让已撤销设备继续保持控制连接。 |
...
**收口结果：** `docs/plans/2026-07-31-001-refactor-w4-shared-javascript-layer-plan.md` 已完成。`api-client`、`app-core`、`ui`、`platform-contract` 四包和公开 exports 已落地；工作项详情协作由共享 use case/UI 驱动，Browser Cookie/CSRF、history、EventSource、File 与 DOM 下载留在 Browser adapters，并由 `web/src/main.jsx` 统一注入。根前端检查、生产构建、19 项 Browser E2E、React 单例检查及 linux/amd64 API 镜像 smoke 通过。W4 不包含 Desktop renderer、device-session、`app://`、Desktop SSE、file-transfer RFC、离线能力、资料库、文档预览、富文本高级体验或旧 Askama 下线。
...
- 由 `web/src/main.jsx` 建立 Browser composition root，通过 `<App services={{ api, events, files, router }} />` 注入宿主能力，并以 JSDoc 明确参数；本阶段不创建 Electron renderer，也不以 Electron 需求改变已验证 Web 行为。
...
- Browser Cookie transport 不能复用到 `app://`。Desktop REST/SSE 使用主进程受控的 Bearer transport：SSE 为附带短期 header 的 fetch-stream，禁止 URL token；network service 只从构建内固定官方 HTTPS origin 和固定 discovery/path registry 构造请求，renderer 不能传递 URL，所有 REST/SSE 请求禁止重定向且不携带 Cookie。生产包禁止环境变量、网页、renderer、CLI 或本地可写文件静默切换 endpoint；renderer CSP 继续保持 `connect-src 'none'`。网络使用 Electron Chromium session、系统代理/PAC 和 OS/企业受控信任库；禁止 `certificate-error` bypass，TLS 过期/SAN 不匹配、407 和 endpoint/server instance 变化均有明确拒绝/重新认证流程。任意私有化 endpoint、签名 enrollment bundle、MDM 根、证书 pinning和私有 CA 导入另立产品与安全计划。
...
- D1-B 的 Device Session control stream 同时绑定 device/user/authorization version 与 access 到期上限。设备撤销、登出、用户禁用或 authorization version 推进后，服务端必须在配置化的 `revocation_close_deadline` 内主动关闭已建立的控制流；超过 deadline 强制关闭，客户端只有完成 refresh/重新授权后才可建立新流。该控制流不包含业务数据。角色、权限和项目成员资格变化后的业务 stream 授权重验与主动断流由 D2 对每个 feature 单独验收。
...
- 打包态覆盖设备授权的开始、批准、取消、超时、账户切换、重启恢复、credential matrix 的 PAT/device/Cookie 拒绝与混合凭证优先级、refresh 单飞/发送前 pending transaction ID 原子持久化/带 transaction ID 的 rotation、服务端 CAS/同 ID 幂等/不同 ID 重放、服务端已轮换后的凭证库写入失败/进程强杀/响应丢失/旧响应迟到恢复、已认证会话探针、固定 endpoint 的 Device Session fetch-stream control SSE 建立/轮换/断开/拒绝及 `revocation_close_deadline` 内撤销后主动关闭路径、登出、CSP、SPA 深链接、导航/重定向拦截、内部路由和 renderer manifest；业务 feature 的读写、角色/成员资格授权变化和业务 SSE 行为只在 D2 对齐 E2E 中验收。
- 负向测试覆盖错误服务器 endpoint、篡改 discovery/server instance/DNS、HTTP 降级、跨/同 origin 重定向、代理 `Location`、TLS 过期/SAN 不匹配/407、refresh 重放、设备撤销/用户禁用后的 REST/SSE 拒绝与控制流关闭、用户切换、登出清理、系统凭证库不可用、第二实例、休眠/唤醒、renderer/主进程崩溃、伪造 IPC payload、非顶层或非 `app://` sender、路径穿越、未许可导航、`window.open` 和权限请求；renderer 不得获得长期凭证。D2 业务流另覆盖权限/角色/成员资格变化。文件测试另覆盖选取后替换、symlink/reparse point、目录/FIFO、字节数/digest 变化、错误 storage host/path/header 和 302，且不泄漏路径或 token。
