...
| `docs/plans/2026-08-02-002-feat-d1-desktop-file-capability-transfer-plan.md` | D1-C 文件 Capability/Transfer 子计划/RFC | `completed` | 主进程短期文件 capability、受控 transfer grant、上传/下载执行器、IPC 最小合约和三平台文件系统负向验证；不开放业务 API 或完整业务 UI。 | 已由本地完整矩阵、正式包文件 smoke 和三平台四 runner Desktop Security Gate 收口，并作为已完成 D2 的安全底座维护。 |
| `docs/plans/2026-08-03-001-feat-d2-desktop-feature-parity-plan.md` | D2 Desktop 业务功能对齐子计划 | `completed` | 共享 App、受限业务 transport、工作项协作、附件、消息/通知、正式包 E2E 与异常恢复。 | 已由各 feature checkpoint、最终 review 和 macOS、Ubuntu、Windows x64、Windows ARM64 同一 Desktop Security Gate 收口；后续进入独立 W3、G-DIST 或 D3/D4 计划。 |
...
Desktop 后续工作统一按下表管理。D1-A 至 D1-C 和 D2 已完成；后续按 W3、G-DIST 或 D3/D4 的独立依赖与退出条件另建可执行子计划，不把六个阶段作为单个执行目标，也不并行突破安全边界。
