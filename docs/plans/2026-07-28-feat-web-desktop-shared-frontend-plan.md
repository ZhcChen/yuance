...
| `docs/plans/2026-08-02-002-feat-d1-desktop-file-capability-transfer-plan.md` | D1-C 文件 Capability/Transfer 子计划/RFC | `ready` | 主进程短期文件 capability、受控 transfer grant、上传/下载执行器、IPC 最小合约和三平台文件系统负向验证；不开放业务 API 或完整业务 UI。 | 当前唯一可执行 Desktop 子计划；完成并复核后再规划 D2 业务功能对齐。 |
...
| D1 / D2：Electron 安全宿主与功能对齐 | `active`（D1-C ready） | 设备认证、`app://` 安全宿主和 Desktop Network/SSE 均已落地；正式包通过协议、网络、生命周期、凭证和三平台 Gate。 | 执行 `docs/plans/2026-08-02-002-feat-d1-desktop-file-capability-transfer-plan.md`；D2 业务对齐和发行 Gate 继续保持独立。 |
| G-DIST / D3 / D4：更新与离线能力 | `pending` | 未启动。 | 作为 D2 后独立 Gate 或离线专项，不阻塞 W3。 |

### Desktop 六阶段路线图

Desktop 后续工作统一按下表管理。D1-A 已完成；其余阶段当前只冻结目标、依赖和退出条件。进入某阶段前必须基于前一阶段的真实实现另建可执行子计划，不把六个阶段作为单个执行目标，也不并行突破安全边界。
...
| 3 | D1-C：文件 Capability / Transfer | `ready` | D1-B 的认证网络边界稳定 | renderer 只持短 TTL capability；主进程验证文件 identity 和 transfer grant；路径、token、重定向、替换与 symlink/reparse point 负向测试通过 | `docs/plans/2026-08-02-002-feat-d1-desktop-file-capability-transfer-plan.md` |
