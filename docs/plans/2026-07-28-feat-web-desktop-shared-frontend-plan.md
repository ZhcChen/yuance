...
| `docs/plans/2026-08-01-002-feat-d1-app-protocol-secure-host-plan.md` | D1 `app://` 安全宿主子计划/RFC | `completed` | 内置 renderer、受限协议、CSP/导航、最小 preload sender 校验、Desktop composition root 与本地 Shell；不包含业务网络、文件 transfer 或发行。 | 已由本地完整矩阵、实际 macOS bundle 和三平台 Desktop Security Gate 收口；D1-B 已另建 ready 子计划。 |
| `docs/plans/2026-08-02-001-feat-d1-desktop-network-sse-plan.md` | D1 Desktop Network/SSE 子计划/RFC | `ready` | 固定 endpoint enrollment、正式 credential runtime、主进程 Device Bearer REST/fetch-stream SSE、撤销断流与脱敏 renderer 状态；不包含文件或完整业务迁移。 | 当前唯一可执行 Desktop 子计划；完成并复核后再规划 D1-C 文件 Capability / Transfer。 |
...
| D1 / D2：Electron 安全宿主与功能对齐 | `active`（D1-B ready） | 设备认证基础与 `app://` 内置安全宿主均已落地；正式包固定加载本地 Shell，协议/CSP/导航/权限/IPC/ASAR 通过三平台 Gate。 | 执行 `docs/plans/2026-08-02-001-feat-d1-desktop-network-sse-plan.md`；file-transfer 和发行 Gate 继续保持独立。 |
...
| 2 | D1-B：Desktop Network / SSE | `ready` | D1-A 通过 review | REST Bearer transport 与 fetch-stream SSE 只由主进程受控 endpoint 发起；无 Cookie、URL token、redirect；轮换、撤销和活跃流关闭有可重复验证 | `docs/plans/2026-08-02-001-feat-d1-desktop-network-sse-plan.md` |
