...
真实使用反馈证明，仅更新 CLI 不能覆盖对象存储签名返回 `Content-Length` 的服务端契约差异。因此本计划的 U2/U3 实施同时修正 API：加密附件上传签名返回协议计算的密文长度，并把该长度绑定到签名请求；CLI 继续执行服务端值与本地协议公式的交叉校验。该补充不改变上传范围，但意味着正式发布必须先更新 API，再发布 `yuance-agent-v0.1.3`。
