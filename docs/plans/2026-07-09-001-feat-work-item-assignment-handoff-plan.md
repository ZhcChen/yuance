...
- R5. 顶部“需求 / 任务 / Bug”Tab 对当前用户待处理数量显示角标，数量大于 99 时显示 `99+` 或等价短文案。
- R6. 角标只统计当前负责人是当前用户、且仍需当前用户处理的开放工作项；处理完成、关闭/取消或指派给下一个人后角标消失。
...
- 是否需要复杂流程引擎：不需要。
- 角标是否点击详情即消失：不消失；只有实际处理完成、关闭/取消或改派后消失。
- 角标最多显示多少：最多显示 99，超过以 `99+` 或等价短文案展示。
...
- 系统级 `work_item.manage` 保留用于系统级管理能力，例如删除、恢复或未来批量维护；如果本轮为了用户体验需要，也可以先对删除/恢复继续使用现有口径。
...
- 布局模板给需求、任务、Bug 链接追加 badge；0 不显示，1-99 显示数字，超过显示 `99+`。
...
- 权限放宽风险：
  - 缓解：只放宽项目内内容写入；删除、恢复等高风险动作可继续保持更严格权限，测试覆盖 viewer 和非成员。
- 角标口径误解：
