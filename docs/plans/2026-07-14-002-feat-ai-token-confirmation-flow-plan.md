...
- R3. 工作项新增“待确认”状态，属于非终态处理状态，纳入待处理统计、角标和列表。
...
- **待确认属于待处理统计口径中的非终态。** 统计、角标、列表的待处理聚合口径应包含待确认；`completed_at` 不能因为待确认而写入。
...
**Approach:**
- 扩展工作项状态校验、状态标签、状态转移、筛选统计中的待处理统计口径判断。
...
- Happy path: 待确认仍被 `status=pending` 统计进待处理集合。
