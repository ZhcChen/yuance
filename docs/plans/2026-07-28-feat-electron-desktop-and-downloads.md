...
### Unit 5：开发运行态隔离与 macOS 开发图标（已完成）

- 开发态使用 `元策 Dev` 的独立 `userData` 与 `sessionData` 路径，正式包保持 `元策` 的默认数据目录。
- 为开发态使用独立应用标识，避免 Windows shell 集成与正式包混用。
- 为 macOS 开发 Dock 图标加入右下角圆点；正式打包继续使用生产 `.icns` 图标。
- 将两份 PNG 作为运行时资源打入内部预览包，并覆盖身份、存储路径、图标选择与构建配置测试。

### Unit 6：登录页精简与默认最大化启动（已完成）

- 移除登录页的环境标识与“统一入口 /web”说明，将用户名提示改为“请输入用户名”，不影响首次初始化入口和登录错误提示。
- Electron 主窗口创建后默认最大化，保留标题栏、Dock 和操作系统标准窗口控制。
- 覆盖登录页渲染与 Electron 窗口配置的聚焦验证。
