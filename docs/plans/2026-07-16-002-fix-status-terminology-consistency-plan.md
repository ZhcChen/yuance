...
本轮聚焦一个核心目标：前台和 Web 统计层不再使用旧抽象状态术语，统一改成用户能直接理解的具体状态表达：
...
- 旧表述属于内部化、抽象化表达，用户阅读时不直观。
- 现有前台不同页面对同一统计口径的表述不一致，有的仍停留在旧抽象语义，有的写“未关闭”，有的已经改成具体状态。
...
- 2026-07-16 已完成验证：
  ...
  - `cargo test summaries_exclude_cancelled_items_from_active_counts`
  - `cargo test my_summary_only_counts_active_high_priority_items`
  - `git diff --check`

## Verification Focus

- 前台不再出现旧抽象状态术语，统一使用具体状态范围表述。
- 页面统计展示统一为具体状态范围。
