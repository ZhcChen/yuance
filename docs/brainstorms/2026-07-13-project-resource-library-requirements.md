...
- R15. 第一阶段不保存明文密码、AccessKey Secret、私钥等高敏感密钥；如需要保存敏感值，应另行设计加密、脱敏、授权和审计能力。
- R16. 资料创建、编辑、归档应记录项目动态或审计信息，便于追溯谁在什么时候变更了资料。

**富文本复用**
- R17. 资料库新建/编辑不得重新实现一套割裂的富文本交互，应复用或提炼现有富文本编辑器。
- R18. 提炼后的富文本组件应能服务至少三类场景：工作项新建说明、工作项讨论/回复、项目资料正文。
...
- 第一阶段不设计高敏感密钥保险箱能力；敏感密钥保存、加密、授权和脱敏单独评估。
...
- [Affects R16][Technical] 资料编辑历史第一期只记录项目动态/审计，还是需要保存可查看的历史版本。
- [Affects R17][Technical] 现有富文本编辑器应提炼到模板 partial、前端初始化函数约定，还是进一步抽象成更通用的 composer 组件。
