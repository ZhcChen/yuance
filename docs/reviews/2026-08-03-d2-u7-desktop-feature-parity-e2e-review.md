...
- Linux packaged network smoke 以 `messageEvidence: integration-fallback` 明确记录平台替代证据：恢复会话证明 SSE connected、refresh 和 revoke，固定 fact 驱动 packaged notification controller 完成真实通知查询；同一 Gate 的真实 Electron business integration 继续强制收到 SSE topbar/release。macOS 与 Windows 使用 `messageEvidence: packaged-sse`，直接在恢复会话中验证完整链路。
