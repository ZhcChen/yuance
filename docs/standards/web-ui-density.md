...
- 页面展示不得直接暴露内部枚举码：工作项优先级 `P0/P1/P2/P3` 展示为 `紧急/高/中/低`，附件状态 `pending/uploaded/deleted` 展示为 `待上传/已上传/已删除`，项目成员角色展示为 `项目负责人/项目管理员/项目成员/只读成员`；表单 value 和 API 契约仍保留稳定英文或代码枚举。
...
- `work_item.manage` 保留给删除、恢复、项目附件等更偏系统维护或高风险动作；无论是否具备 RBAC 权限，项目 `viewer` 和非成员都不能写入项目内容。
