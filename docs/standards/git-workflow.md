...
- 本项目默认在当前检出的协作分支开发；若当前分支已跟踪远端（例如 `dev`），提交后推送到该当前分支。除非用户另有说明，不额外创建功能分支。
- 每轮对话完成一个小逻辑、小功能或小修复后，默认必须提交并推送。
- 提交单位不是“消息次数”，而是“最小可解释业务闭环”。
- 提交前只暂存本轮相关文件，禁止默认使用 `git add .`。
- commit 成功后立即推送当前分支，避免本地长期堆积未推送工作。
...
如果当前分支没有远端上游，或工作区存在无关未提交改动导致无法安全 `rebase`，不要为了推送本轮提交而 stash、回滚或提交无关改动；可以跳过 `rebase`，直接推送当前分支，并在最终回复中说明原因。

如果推送失败：

- 不静默跳过。
- 先判断是网络、认证、远端拒绝还是需要拉取 / rebase。
