...
5. 资料正文或附件写操作前读取资料详情和附件列表；首次上传按“确认本地文件与目标 -> `attachments upload --file` -> 更新正文引用”执行；替换附件时完成新附件后再更新正文，确认引用后才删除旧附件。
...
- 创建工作项前必须明确 `project_key`、`item_type` 和 `title`；创建资料前必须明确 `project_key` 和 `title`。
...
当前支持项目、工作项、工作项评论、资料创建/查询/更新、资料附件登记与受控本地文件上传、签名请求/完成登记/条件删除和通知查询。文件下载、资源附件预览 CLI、设备会话、system OpenAPI、工作项批量操作、保存视图和通用 HTTP 代理仍不支持。用户要求不支持的能力时停止，不猜测隐藏命令或直接调用未知端点。
