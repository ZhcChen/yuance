...
当前支持项目、工作项、工作项评论、资料创建/查询/更新、资料附件登记/签名请求/完成登记/条件删除和通知查询。对象存储文件字节上传/下载、资源附件预览 CLI、设备会话、system OpenAPI、工作项批量操作、保存视图和通用 HTTP 代理仍不支持。用户要求不支持的能力时停止，不猜测隐藏命令或直接调用未知端点。
